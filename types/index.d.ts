...
type IRenderListViewProps<T> = Omit<
    Omit<Omit<IPropsSwipeListView<T>, 'useFlatList'>, 'useSectionList'>,
    'renderListView'
>;

type RowMap<T> = { [open_cell_key: string]: SwipeRow<T> };

type cellPropsBuilderP<T> = Pick<
    IPropsSwipeRow<T>,
    | 'onRowOpen'
    | 'onRowDidClose'
    | 'onRowDidOpen'
...
interface IPropsSwipeListView<T> {
    cellPropsBuilder: cellPropsBuilderP<T>;
    // data: T[];
    // sections: Array<{
    // 	title: string;
    // 	data: T[];
     ...
     */
    renderListView(
        props: IRenderListViewProps<T>,
        setRefCallback: unknown,
        onScrollCallback: (
            event: NativeSyntheticEvent<NativeScrollEvent>
        ) => void,
        renderItemOrRowCallback:
            | ((
...
interface IUseSectionListProps<T>
    extends SectionListPropsOverride<T>,
        IPropsSwipeListView<T> {
    useSectionList: boolean;
}

type FlatListPropsOverride<T> = Omit<FlatListProps<T>, 'renderItem'>;

interface IUseFlatListProps<T>
    extends FlatListPropsOverride<T>,
        IPropsSwipeListView<T> {
    useFlatList: boolean;
}

export class SwipeListView<T> extends Component<
    Partial<IUseSectionListProps<T>> | Partial<IUseFlatListProps<T>>
> {
